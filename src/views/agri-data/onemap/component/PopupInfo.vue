<template>
  <div>
    <div class="right_tab_container" v-if="data && data.dialogType == '农机'">
      <div class="title">
        <strong>农机详情</strong>
    ...
    <div class="right_tab_container" style="top: 0px" v-if="data && data.dialogType == '合作社'">
      <div class="title">
        <strong>合作社详情</strong>
        <div class="close-container"><img src="@/assets/images/one-picture-close.png" @click="close"></div>
      </div>
      <div class="content">
        <div v-for="(item,index) in cooperativeList" class="info" :key="index">
          <div class="key-container">{{ item.name }}</div>
          <div class="value-container" :class="item.class">{{ item.value }}</div>
        </div>
      </div>
    </div>
    <div class="right_tab_container" style="top: 0px" v-if="data && data.dialogType == '维修站点'">
      <div class="title">
        <strong>维修站点详情</strong>
        <div class="close-container"><img src="@/assets/images/one-picture-close.png" @click="close"></div>
      </div>
      <div class="content">
        <div v-for="(item,index) in pitList" class="info" :key="index">
          <div class="key-container">{{ item.name }}</div>
          <div class="value-container" :class="item.class">{{ item.value }}</div>
        </div>
      </div>
    </div>
    <div class="right_tab_container" style="top: 0px" v-if="data && data.dialogType == '加油站'">
      <div class="title">
        <strong>加油站详情</strong>
        <div class="close-container"><img src="@/assets/images/one-picture-close.png" @click="close"></div>
      </div>
      <div class="content">
        <div v-for="(item,index) in gasStationList" class="info" :key="index">
          <div class="key-container">{{ item.name }}</div>
          <div class="value-container" :class="item.class">{{ item.value }}</div>
        </div>
      </div>
    </div>
    <div class="right_tab_container" v-if="data && data.dialogType == '销售站点'">
      <div class="title">
...
<script>
import {getMachineDetail, getProductDetail} from "../../../../api/agridata";
...
export default {
  ...
  data() {
    return {
      ...
      //销售站点详情
      saleList: [
        {id:1, name: '名称：',value: '', class: '', relation: 'groupNameFullPath',},
        {id:2, name: '地址：',value: '', class: '', relation: 'groupNameFullPath',},
        {id:3, name: '联系人：',value: '', class: '', relation: 'groupNameFullPath',},
        {id:4, name: '联系电话：',value: '', class: '', relation: 'groupNameFullPath',},
      ]
  ...
  watch: {
    "data": {
      handler(newName, oldName) {
        if(newName.resoucesType == 'machine'){
