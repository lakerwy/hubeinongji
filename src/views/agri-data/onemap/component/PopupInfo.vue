<template>
    <div>
        <div class="right_tab_container" :class="{'leftClass':leftstyle,'topClass':topstyle,'bottomClass':bottomstyle}" v-if="data && data.dialogType == 'machine'">
            <div class="title">
        ...
        <div class="right_tab_container" v-if="data && data.dialogType == 'trackPoint'">
            <div class="title">
                <strong>轨迹点详情</strong>
                <div class="close-container"><img src="@/assets/images/one-picture-close.png" @click="close"></div>
            </div>
            <div class="content">
                <div v-for="(item,index) in trackPointList" class="info" :key="index">
                    <div class="key-container">{{ item.name }}</div>
                    <div class="value-container" :class="item.class">{{ item.value }}</div>
                </div>
            </div>
...
<script>
    ...
    export default {
        props: ["selectId", "data","leftstyle","topstyle","bottomstyle"],
        components: {},
        data() {
            return {
                //农机详情
                ...
                //轨迹点详情
                trackPointList: [
                    {id: 1, name: '时间：', value: '', class: '', relation: 'Time',},
                    {id: 2, name: '经度：', value: '', class: '', relation: 'XCoord',},
                    {id: 3, name: '纬度：', value: '', class: '', relation: 'YCoord',},
                    {id: 4, name: '深度(cm)：', value: '', class: '', relation: 'jobHight',},
                    {id: 5, name: '速度(km/h)：', value: '', class: '', relation: 'velocity',},
                    {id: 6, name: '农机类型：', value: '', class: '', relation: 'machineType',},
                    {id: 7, name: '机手：', value: '', class: '', relation: 'ownerName',},
                ],
                machineTile: "",
            }
        },
        methods: {
            ...
            async getTrackPointDetail(data) {
                //console.log("data",data);
                if(data.attributes){
                    this.trackPointList.forEach(item => {
                        item.value = data.attributes[item.relation];
                    })
                }
        ...
        watch: {
            "data": {
                handler(newName, oldName) {
                    ...
                    } else if (newName.dialogType == 'trackPoint') {
                        this.getTrackPointDetail(newName)
                    }
                },
                immediate: true
            },
