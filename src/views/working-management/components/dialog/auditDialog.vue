<template>
  <el-dialog 
    ...
    <el-form class="reason-form" ref="form" :model="form" label-width="150px" label-position="top">
      ...
      <el-form-item label="审核不通过原因:" v-if="form.auditFlag == 2">
        <el-checkbox-group v-model="form.reason">
          <el-checkbox style="display: block;" v-for="item in reasons" :label="item.itemCode" :key="item.itemCode">{{item.itemName}}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
    </el-form>
