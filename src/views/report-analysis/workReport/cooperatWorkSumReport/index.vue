...
import { queryCooperationJobSummary , exportCooperationJobSummary} from "_api/report/work";
...
export default {
  ...
  ...mapMutations({
    ...
    getBtnPermis() {
      this.btnPermis.btnView = this.permissions[window.global.buttonPremission.cooperatWorkSumReportView];
      this.btnPermis.btnExport = this.permissions[window.global.buttonPremission.cooperatWorkSumReportExport];
    ...
    async initData() {
      this.loading = true
      let res = await queryCooperationJobSummary({
    ...
    async exportExcel(){
      this.loading = true
      let res = await exportCooperationJobSummary({
        page:this.page.currentPage,
        rows:this.page.pageSize,
        groupId: this.groupData.children.toString(),
        bTimeEx: dateFormat(this.form.bTimeEx),
