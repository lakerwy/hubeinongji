...
import { queryWorkAcceptance , exportWorkAcceptance } from "_api/report/work";
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        { prop: "driverName", label: "作业机手姓名"},
        { prop: "address", label: "作业地点" },
        { prop: "jobTime", label: "作业时间" },
        { prop: "computeArea", label: "作业面积（亩） " },
        { prop: "avgDepth", label: "作业质量（厘米）" },
        { prop: "driverPhone", label: "作业机手联系电话" },
        { prop: "", label: "作业机手签字" },
      ],
      tableData: [],
      jobTypeSelect:[{
  ...
  methods: {
    ...
    async initData() {
      this.loading = true
      let res = await queryWorkAcceptance({
        currentPage:this.page.currentPage,
        pageSize:this.page.pageSize,
        machineId: this.groupData.expand[0],
    ...
    async exportExcel(){
      this.loading = true
      let res = await exportWorkAcceptance({
        currentPage:this.page.currentPage,
        pageSize:this.page.pageSize,
        machineId: this.groupData.expand[0],
        bTimeEx: dateFormat(this.form.bTimeEx),
