...
import {queryJobAssistanceSituationSummary,exportJobAssistanceSituationSummary} from '_api/report/subsidy';
import {getMachineDict} from "@/api/basic/machine";
import { mapGetters, mapMutations  } from 'vuex'
import { handBlobDown ,filterDict ,dateFormat} from '@/util/util'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { prop: "driverPhone", label: "农机手号码 "},
        { prop: "groupNameFullPath", label: "农机手地址",with:"500px"},
  ...
  methods: {
    ...
    getBtnPermis() {
      this.btnPermis.btnView = this.permissions[window.global.buttonPremission.jobAssistanceSituationSummaryView];
      this.btnPermis.btnExport = this.permissions[window.global.buttonPremission.jobAssistanceSituationSummaryExport];
    ...
    async initData() {
      this.loading = true
      let res = await queryJobAssistanceSituationSummary({
    ...
    async exportExcel(){
      this.loading = true
        let res = await exportJobAssistanceSituationSummary({
        currentPage: this.page.currentPage,
        pageSize:this.page.pageSize,
        groupId:this.groupData.ids.toString(),
        jobType:this.form.jobType,
