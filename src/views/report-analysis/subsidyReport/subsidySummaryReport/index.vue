...
import {
  queryAssistanceSituationSummary,
  exportAssistanceSituationSummary,
} from "_api/report/subsidy";
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { prop: "address", label: "作业地点(细化到县乡村)" },
  ...
  methods: {
    ...
    getBtnPermis() {
      this.btnPermis.btnView =
        this.permissions[window.global.buttonPremission.assisSummaryView];
      this.btnPermis.btnExport =
        this.permissions[window.global.buttonPremission.assisSummaryExport];
    ...
    async initData() {
      this.loading = true;
      let res = await queryAssistanceSituationSummary({
    ...
    async exportExcel() {
      this.loading = true
      let res = await exportAssistanceSituationSummary({
        currentPage: this.page.currentPage,
        pageSize: this.page.pageSize,
        groupId: this.groupData.ids.toString(),
        jobType: this.form.jobType,
