            ...
            <el-card class="box-card">
                ...
                <div class="groups-con">
                    <el-form class="my-form tree-form" label-position="right" :model="form">
                        <el-form-item>
                            <el-input v-model="form.keyword" placeholder="输入农机或者分组名查找" @input="searchChange">
                                <template #suffix>
                                    <img style="margin-top:8px;margin-right:10px;cursor:pointer;"
                                         src="img/monitoring/icon_query.png" alt="" @click="queryByGroup">
                                </template>
                            </el-input>
                        </el-form-item>
                    </el-form>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async initData() {
                ...
                if (this.treeType === 1) {
                    ...
                    if (this.isAll) {
                        ...
                        this.data = toTreeData(res.data, 'id', 'pId');
                    } else {
                ...
                if (this.groupData && this.groupData.ids) {
                    ...
                    this.defaltCheckedKeys = this.groupData.ids.length > 0 ? this.groupData.data.map(item => {
                        return item.id
                    }) : [];
                }
            },
            ...
            submit() {
                // 保存数据
                this.setGroupData({name: this.current.name, ids: [this.current.id], data: this.current});
                this.setGroupBoxStatus(false);
            }
        }
    }
